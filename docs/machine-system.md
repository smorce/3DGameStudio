...
Compileはfixed接続の連結成分ごとに複合剛体を作り、接続していない部品を別剛体にします。HingeはRapierのRevolute Jointへ変換します。SuspensionはColliderを持たず、質量だけを複合剛体へ加えます。WheelはRaycast Vehicleの車輪として実行し、サスペンション長・転がり・操舵を描画へ反映します。車輪の側面Colliderはありません。
