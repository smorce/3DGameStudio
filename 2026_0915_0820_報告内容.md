...
- `tests/e2e/observation-agent.spec.ts`
  ...
  - 11 Scenario smoke：begin→短時間play→events→end
...
今回の差分範囲では上記を確認しています。フル `pnpm test:e2e` / `pnpm smoke` の全件再走は未実施です（11 Scenario smoke テストは追加済み）。
...
それでも、失敗時こそ証拠を残す、同じ Physics Step で同じ入力を再現する、観測自身の負荷を抑える、GPU を Summary まで届ける、という指示16の Approve 条件に必要な残り項目は実装済みです。
